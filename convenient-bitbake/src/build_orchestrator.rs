...
impl BuildOrchestrator {
    ...
    fn resolve_file_uris(
        ...
        if recipe_dir.is_none() {
            trace!("Could not find recipe directory for {}", recipe_name);
            ...
            if let Some(file_uri) = part.strip_prefix("file://") {
                ...
                for search_dir in &search_paths {
                    ...
                    if candidate.exists() && candidate.is_file() {
                        trace!("Resolved file://{} → {}", filename, candidate.display());
                ...
                if !file_resources.contains_key(filename) {
                    trace!("Could not resolve file://{} in FILESPATH for {}", filename, recipe_name);
                }
            }
        }
